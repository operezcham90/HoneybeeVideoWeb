...
var hb = {
    ...
    delta: function (u, l, h) {
        ...
        if (u < 0.5) {
            aa = 2 * u + (1 - 2 * u) *
                    Math.pow((1 + l), (hb.ini.eta.m + 1));
            d = Math.pow(aa, (1 / (hb.ini.eta.m + 1))) - 1;
        } else {
            aa = 2 * (1 - u) + 2 * (u - 0.5) *
                    Math.pow((1 - h), (hb.ini.eta.m + 1));
    ...
    mutation: function (i) {
        // for each site
        for (var s = 0; s < hb.pop.arr[hb.pop.lambda][i].dim.length - 2; s++) {
            // get value
            var x = hb.pop.arr[hb.pop.lambda][i].dim[s];
            var r = hb.limits[s].max - hb.limits[s].min;
            if (s >= 4) {
                // dependent components
                r -= hb.pop.arr[hb.pop.lambda][i].dim[s - 2];
            }
            // get distance min
            ...
            if (s >= 4) {
                // dependent components
                h = (hb.limits[s].max - x -
                        hb.pop.arr[hb.pop.lambda][i].dim[s - 2]) / r;
            }
            if (h > 1.0) {
                h = 1.0;
            }
            ...
            // limits
            if (hb.pop.arr[hb.pop.lambda][i].dim[s] < hb.limits[s].min) {
                hb.pop.arr[hb.pop.lambda][i].dim[s] = hb.limits[s].min;
            }
            if (hb.pop.arr[hb.pop.lambda][i].dim[s] > hb.limits[s].max) {
                hb.pop.arr[hb.pop.lambda][i].dim[s] = hb.limits[s].max;
            }
        }
    },
    ...
    offspring: function (size, cross, mut, rand) {
        ...
        for (var i = 0; i < nm; i++) {
            ...
            // mutate
            hb.mutation(hb.pop.arr[hb.pop.lambda].length - 1);
        }
        // crossover
